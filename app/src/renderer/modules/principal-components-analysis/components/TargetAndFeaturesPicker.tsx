...
const TargetAndFeaturesPicker: React.FC<PropsFromRedux> = props => {
  ...
  React.useEffect(() => {
    ...
    const nextStepUnlocked = props.nextStepUnlocked(StepsID.TargetAndFeaturesPicker);

    if (allowNextStep && !nextStepUnlocked) props.unlockNextStep(StepsID.TargetAndFeaturesPicker);
    else if (!allowNextStep && nextStepUnlocked) props.lockNextStep(StepsID.TargetAndFeaturesPicker);
  }, [props.target, props.features]);

  // fetch possible targets & features
  React.useEffect(() => {
    if (props.possibleValues.features.length === 0) props.fetchPossibleTargetsAndFeatures();
  }, []);
